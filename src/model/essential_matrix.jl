...
#FundamentalMatrix(model₁::AbstractCameraModel, model₂::AbstractCameraModel, reference_system::AbstractCoordinateSystem) = FundamentalMatrix(construct_fundamental_matrix(model₁, model₂, reference_system))
...
function construct_essential_matrix(camera₁::AbstractCamera, camera₂::AbstractCamera,  reference_system::AbstractCoordinateSystem)
    ...
    𝐊₂ = to_matrix(intrinsics₂, image_system₂)
    𝐅 = matrix(FundamentalMatrix(camera₁, camera₂, reference_system))
    𝐄 = 𝐊₂'*𝐅*𝐊₁
end
